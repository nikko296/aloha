...
require ("koneksi.php");
       ...
       if (!mysql_select_db($dbname,$link))
        ...
        // proses seleksi, bila data sudah ada tdk disimpan.
         $hasil1 = mysql_query("select * from master_hw WHERE master_hw.keterangan like '$keterangan'", $link);

       //  if ( $jml_baris = mysql_num_rows($hasil1) > 0 )  
        // die("Data dengan ALAMAT IP = $keterangan sudah ada di dalam database, silahkan teliti kembali proses yang anda lakukan!");

        // simpan data
        

        $hasil = "UPDATE master_hw SET no_sn='$no_sn',jenis_hw='$jenis_hw',merk_hw='$merk_hw',type_hw='$type_hw',spesifik_hw='$spesifik_hw',tgl_perolehan='$tgl_perolehan',nilai_perolehan='$nilai_perolehan',lokasi='$lokasi',pemilik='$pemilik',nik='$nik',keterangan='$keterangan' WHERE no_sn='$no_sn'";
echo mysql_error();
        mysql_query($hasil);
        if (mysql_affected_rows() > 0)
             print("Data dengan ALAMAT IP = $keterangan sudah dihapus dari dalam database dengan benar dan sukses");
        else
             print("Data dengan ALAMAT IP = $keterangan gagal dihapus dari dalam database");
        mysql_close($link);
  ?>
  <tr>
