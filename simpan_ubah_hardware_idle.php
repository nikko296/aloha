...
require ("koneksi.php");
       ...
       if (!mysql_select_db($dbname,$link))
        ...
        // proses seleksi, bila data sudah ada tdk disimpan.
         $hasil1 = mysql_query("select * from table_ggn_hw WHERE table_ggn_hw.no_sn like '$no_sn'", $link);

       //  if ( $jml_baris = mysql_num_rows($hasil1) > 0 )  
     //    die("Data dengan NOMOR SERIAL = $no_sn sudah ada di dalam database, silahkan teliti kembali proses yang anda lakukan!");

        // simpan data
        

        $hasil = "UPDATE table_ggn_hw SET no_sn='$no_sn',jenis_hw='$jenis_hw',merk_hw='$merk_hw',type_hw='$type_hw',spesifik_hw='$spesifik_hw',tgl_ggn='$tgl_ggn',jns_ggn='$jns_ggn',inst_perbaikan='$inst_perbaikan',almt_inst='$almt_inst',ptgs_penerima='$ptgs_penerima',biaya='$biaya',tgl_selesai='$tgl_selesai' WHERE no_sn='$no_sn'";
echo mysql_error();
        mysql_query($hasil);
        if (mysql_affected_rows() > 0)
             print("Data dengan NOMOR SERIAL = $no_sn sudah diubah ke dalam database dengan benar dan sukses");
        else
