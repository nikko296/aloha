...
require ("koneksi.php");
       ...
       if (!mysql_select_db($dbname,$link))
        ...
        // proses seleksi, bila data sudah ada tdk disimpan.
           $hasil = mysql_query("select * from tabel_pegawai where nik like '$nik'", $link);

           if ($jml_baris = mysql_num_rows($hasil) > 0 )  die("Data dengan NIK = $nik sudah ada di dalam database, silahkan teliti kembali proses yang anda lakukan!");


        // simpan data
        $hasil = "INSERT INTO tabel_pegawai (nik,nama,jabatan,loker) values ('$nik','$nama','$jabatan','$loker')";

        mysql_query($hasil,$link);
        if (mysql_affected_rows() >= 0)
             print("Data dengan NIK = $nik sudah disimpan ke dalam database dengan benar dan sukses");
        else
